...
当物理内存中没有空闲页面时，操作系统必须先腾出一个已有页面的位置，才能装入新的页面。这个过程的具体实现因平台而异。
...
**iOS** 则没有内存置换技术，系统会直接将某些不活跃的帧清空！也就是进程在后台被系统杀死了。他实际上把页面分成了两类，处理方式不同：
...
在 arm64/iOS 上，栈是向低地址增长的：每次调用新函数，栈顶指针（`SP`）向更小的地址移动来"压入"新的栈帧；函数返回时 `SP` 往回移动，栈帧随之弹出。这个方向和堆刚好相反，是后面区分两者的一个关键点。
...
堆是运行时动态申请的内存区域，用于存储生命周期需要超越当前函数调用的数据。你必须显式地从堆中请求分配内存；如果不使用垃圾回收机制，还需要显式地释放它。当你调用 `malloc` 和 `free` 时，操作的正是堆内存。
...
- **Active List（活动列表）​** 存放的是当前正被进程映射且近期有过访问记录的页面。这些页面是"热数据"，内核认为它们很快还会被用到，因此不会主动回收。每当一个页面被访问，它就会被标记为"recently touched"，从而留在活动列表中。

- **Inactive List（非活动列表）​** 存放的是仍然驻留在物理内存中、但近期没有被访问的页面。这些页面的数据依然有效，但内核将其视为"冷数据"候选——它们随时可能被回收。非活动列表是内核执行内存压力回收的主要来源。值得注意的是，一个页面从活动列表降级到非活动列表，并不意味着它的数据立刻消失，而是说明内核开始"怀疑"它短期内不再被需要。

- **Free List（自由列表）​** 存放的是完全不属于任何进程地址空间或 VM 对象的物理页面，可以被任何进程立即取用。自由列表的大小是内核判断内存压力的核心指标。

当自由列表的页面数量**低于某个阈值**（这个阈值由物理内存总量决定）时，内核的 pager（分页器）就会启动回收流程，主要目标是从非活动列表中"抢救"或"释放"页面。具体规则如下：

- 如果一个非活动页面**近期被访问过**，说明它其实还是"热"的，内核会将其**重新激活**，挪回活动列表末尾，不予回收。
- 如果一个非活动页面**未被修改**（即内容与磁盘/后备存储一致）且不是 wired（永久驻留）页面，内核直接**销毁其虚拟映射**，将其"偷走"放入自由列表——这个操作几乎是零成本的，因为不需要写磁盘。
- 如果一个非活动页面**已被修改**（dirty page），在 macOS 上，内核必须先将其内容写入 backing store（磁盘交换区），才能将其释放到自由列表。而在 **iOS 上，系统没有 backing store**，因此脏页无法被换出，必须留在内存中，由拥有该页面的应用程序自行清理——这也是 iOS 在内存压力下会直接终止应用（OOM kill）而非换页的根本原因。

与此同时，内核也会从另一个方向补充自由列表：当活动列表中的页面**长期未被访问**时，内核会将其降级到非活动列表，为后续的回收做准备。当进程**显式释放内存**（如调用 `free()`），或者虚拟页面被换出后，对应的物理页面也会直接进入自由列表。

###  **页面换入（Page-In）：软故障与硬故障**
...
处理路径涉及磁盘 I/O：VM 对象的 pager（可能是 default pager 或 vnode pager）需要从 backing store 或磁盘文件中**定位并读取**该页面的数据，更新页表映射信息，将页面载入物理内存，并将其放入活动列表。同样地，若涉及写操作，页面被标记为 modified。

两种故障的判断依据在于内核对 VM 对象**驻留页面列表**的查询：如果目标页面在列表中，产生软故障；不在列表中，则产生硬故障。这个区分使得内核能够用最低代价处理共享内存场景，同时在真正缺页时才付出磁盘访问的代价。
