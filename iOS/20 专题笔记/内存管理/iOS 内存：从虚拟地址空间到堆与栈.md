...
**iOS** 则完全放弃了后备存储。可写数据页面永远不会被换出到磁盘，这是为了保护闪存的有限写入寿命，同时避免换页带来的响应延迟。但**只读页面**（例如可执行代码段）是个例外——由于它们的内容固定不变，系统可以在内存紧张时直接将其丢弃，并在需要时重新从磁盘（即 App 的二进制文件）调入，这在本质上是一种"按需加载"而非传统意义上的换页。
