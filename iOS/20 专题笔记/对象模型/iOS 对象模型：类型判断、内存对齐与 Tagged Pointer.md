...
这篇先不从 Runtime 源码开始。我们从一行每天都会写的代码开始：
...
- `d` 是一个指针变量，它保存的是地址；
- `[Dog new]` 创建出来的才是对象本身，普通情况下它住在堆里。

如果把它们混成一个，后面的 `isa`、对象大小和 Tagged Pointer 都会越看越乱。
...
第一次读时，先读前四部分和最后的“一句话面试答案”。“进阶：源码和实验细节”可以等主线理解以后再看。
